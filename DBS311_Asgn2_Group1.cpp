...
//This function returns 1 if the customer exists. If the customer does not exists, this function returns 0 and the main menu is displayed.
int customerLogin(Connection* conn, int customerId) {
...
//This function returns the number of products (items) entered by the user.
int addToCart(Connection* conn, struct ShoppingCart cart[]) {
	...
	for (int i = 0; i < 5; ++i) {
		...
		try {
			...
			// calling function find product that will return price
...
//The procedure receives the product ID and returns the price. 
//If the price is 0, the product ID is not valid (does not exist). 
//If the price is a non-zero value, it means the product ID is valid.
double findProduct(Connection* conn, int product_id) {
	double price;
	Statement* stmt = conn->createStatement();
	try {
...
//This function askes the user if they would like to checkout as well as if they would like to exit the program.
//This function checks if a proper Yes or No input is entered.
int checkout(Connection* conn, struct ShoppingCart cart[], int customerId, int productCount) {
	char opt;
	Statement* stmt = conn->createStatement();
	do {
...
int main(void) {
	...
	string str;
	string user = "dbs311_211a14";
	string pass = "94472514";
